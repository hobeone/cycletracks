...
xsi:schemaLocation="http://www.topografix.com/GPX/1/1 http://www.topografix.com/GPX/1/1/gpx.xsd">"""
  ...
  def testValidParse10(self):
    ...
    self.assertEqual(act['average_bpm'], 0)
    self.assertEqual(act['average_cadence'], 0)
    self.assertEqual(act['rolling_time'], 49)
    self.assertAlmostEqual(act['average_speed'], 10.14, 1)
    self.assertEqual(act['maximum_bpm'], 0)
    self.assertEqual(act['maximum_cadence'], 0)
  ...
  def testValidParse11_tpx(self):
    testfile = open('gcycle/test/valid_single_segment_11_tpx.gpx').read()
    act = pygpx.parse_gpx(testfile, '1/1')
    self.assertEqual(act['maximum_bpm'], 172)
    self.assertEqual(act['maximum_cadence'], 103)
    self.assertEqual(act['average_bpm'], 134)
    self.assertEqual(act['average_cadence'], 70)
    self.assertFalse('total_calories' in act)
    self.assertEqual(act['end_point'], '37.996776,-122.506612')
    self.assertAlmostEqual(act['average_speed'], 10.14, 1)
    self.assertAlmostEqual(act['maximum_speed'], 18.26, 1)
