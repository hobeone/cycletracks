...
import urllib, logging, os, sys, traceback, time
...
def read_send_receive_states():
    ...
    if cur.fetchone():
        ...
        cur.execute("CREATE TABLE rocket_station (kind VARCHAR(255), send_state VARCHAR(500), receive_state VARCHAR(500), PRIMARY KEY (kind)) CHARACTER SET utf8 COLLATE utf8_general_ci")
...
def send_row(kind, key, entity):
    result = urllib.urlopen("%s/%s" % (ROCKET_URL, kind), urllib.urlencode(entity))
    try:
...
def receive_updates(kind, table_name, timestamp_field, table_key_field, receive_fields):
    ...
    while count == BATCH_SIZE:
        ...
        url = "%s/%s?timestamp=%s&count=%d" % (ROCKET_URL, kind, timestamp_field, BATCH_SIZE)
...
def get_table_metadata(cur, table_name, timestamp_field, table_key_field):
    ...
    if tables.has_key(table_name):
        ...
        if cur.fetchone():
            ...
            # tables is missing
            cur.execute("CREATE TABLE %s (%s VARCHAR(255) NOT NULL, %s TIMESTAMP, PRIMARY KEY(%s), INDEX %s(%s)) CHARACTER SET utf8 COLLATE utf8_general_ci" % (table_name, table_key_field, timestamp_field, table_key_field, timestamp_field, timestamp_field))
...
def create_field(cur, table_name, table_key_field, field_name, field_type, is_list):
    if is_list:
        ...
        list_table_name = "%s_%s" % (table_name, field_name)
        cur.execute("CREATE TABLE %s (id INTEGER NOT NULL AUTO_INCREMENT, %s VARCHAR(255) NOT NULL, PRIMARY KEY(id), INDEX k(%s)) CHARACTER SET utf8 COLLATE utf8_general_ci" % (list_table_name, table_key_field, table_key_field))
        ...
        if field_type == TYPE_DATETIME:
            ...
            cur.execute("ALTER TABLE %s ADD COLUMN `%s` LONGTEXT" % (table_name, field_name))
        elif field_type == TYPE_BLOB:
            cur.execute("ALTER TABLE %s ADD COLUMN `%s` LONGBLOB" % (table_name, field_name))
